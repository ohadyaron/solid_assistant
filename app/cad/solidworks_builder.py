...
def build_sldprt(part: PartIntent, output_dir: Path) -> Path:
    ...
    This function creates a parametric CAD model in SolidWorks using the COM API
    ...
    try:
        ...
        # Create base shape
        if part.shape == "box":
            _create_box_solidworks(sw_model, part.dimensions)
        else:
            raise ValueError(f"Unsupported shape: {part.shape}")
        
        # Apply holes
        for hole in part.holes:
            _add_hole_solidworks(sw_model, hole)
        
        # Apply fillets
        for fillet in part.fillets:
            _add_fillet_solidworks(sw_model, fillet)
...
def _create_box_solidworks(sw_model, dimensions) -> None:
    """
...
def _add_hole_solidworks(sw_model, hole) -> None:
    """
...
def _add_fillet_solidworks(sw_model, fillet) -> None:
    """
